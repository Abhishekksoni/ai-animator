...
import {
  PanelLeft, Plus, Key, ChevronDown,
  SplitSquareVertical, Sun, Moon, LogOut, Bot
} from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className={`h-13 border-b px-3.5 flex items-center justify-between shrink-0 z-20 transition-colors duration-200 ${
      isDark
        ? 'border-[#242428] bg-[#0d0d0f]'
        : 'border-[#e6e4dc] bg-[#fbfbfa]'
    }`}>
      {/* Left section: Sidebar toggle & New Chat & Model Selector */}
      <div className="flex items-center gap-2">
        <button
          ...
          className={`flex h-8 w-8 items-center justify-center rounded-lg transition ${
            isDark
              ...
              : 'text-stone-600 hover:bg-[#edebe4] hover:text-stone-900'
          }`}
        >
        ...
        <button
          ...
          title="Start new animation chat"
          className={`flex h-8 w-8 items-center justify-center rounded-lg transition ${
            isDark
              ? 'text-zinc-400 hover:bg-[#1f1f23] hover:text-zinc-200'
              : 'text-stone-600 hover:bg-[#edebe4] hover:text-stone-900'
        ...
        {/* Model selector dropdown (Claude / ChatGPT style) */}
        <div className="relative ml-1">
          <select
            value={selectedModel}
            onChange={(e) => onSelectModel(e.target.value)}
            className={`appearance-none rounded-xl border pl-3 pr-7 py-1.5 text-xs font-medium focus:border-[#d97736] focus:outline-none focus:ring-1 focus:ring-[#d97736] cursor-pointer transition ${
              isDark
                ? 'border-[#2e2e33] bg-[#17171a] hover:bg-[#1f1f23] text-zinc-200'
                : 'border-[#dcd9ce] bg-white hover:bg-[#f5f4ef] text-stone-800 shadow-xs'
            }`}
          >
            {models.map((m) => (
              <option
      ...
      {/* Right section: Theme Toggle, Canvas, API Keys & Google Auth Button */}
      <div className="flex items-center gap-2">
        {/* Theme Toggle Button */}
        <button
        ...
        {hasArtifact && (
          <button
            ...
            title={isArtifactOpen ? 'Close canvas' : 'Open canvas'}
        ...
        {user ? (
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className={`flex items-center gap-1.5 rounded-xl p-1 transition border ${
                isDark
          ...
          <button
            onClick={onOpenAuth}
            className={`flex items-center gap-2 rounded-xl border px-3 py-1.5 text-xs font-semibold transition cursor-pointer active:scale-95 shadow-xs ${
              isDark
